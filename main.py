...
if __name__ == '__main__':
    ...
    # initialize the ice
    ice_true = Ice(l_abs=settings.L_ABS_TRUE, l_scat=settings.L_SCAT_TRUE)

    ice_pred = Ice(trainable=True, l_abs=settings.L_ABS_START,
    ...
    for i in range(len(settings.L_ABS_TRUE)):
        ...
        # l_scat
        avg_t_true = \
            detector.tf_average_arrival_times(model_true.arrival_times,
                                              model_true.final_positions)
        avg_t_pred = \
            detector.tf_average_arrival_times(model_pred.arrival_times,
                                              model_pred.final_positions)

        # make sure DOMs with no hits don't contribute
        avg_t_true = tf.where(avg_t_pred < 0., -tf.ones_like(avg_t_true),
                              avg_t_true)
    ...
    accumulated_gradients = [tf.Variable(tf.zeros_like(tv.initialized_value()),
                                         trainable=False) for tv in variables]
    ...
    apply_gradients = optimizer.apply_gradients([
        ...
        in zip(accumulated_gradients, variables)])
    ...
    for step in range(1, settings.MAX_STEPS + 1):
        ...
        result = session.run(ice_pred.l_abs + ice_pred.l_scat)
        logger.log(step, result)

        # reset variables for next step
        session.run(reset_gradients)
